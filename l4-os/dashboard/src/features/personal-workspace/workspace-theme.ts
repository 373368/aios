...
export type WorkspaceTheme = "loopx" | "paper" | "brutal" | "aios";

export const defaultWorkspaceTheme: WorkspaceTheme = "aios";

export function readWorkspaceTheme(): WorkspaceTheme {
  try {
    const stored = window.localStorage.getItem(workspaceThemeStorageKey);
    return stored === "loopx" || stored === "paper" || stored === "brutal" || stored === "aios"
      ? stored
