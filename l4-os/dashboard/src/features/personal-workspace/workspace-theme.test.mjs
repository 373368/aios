...
assert.match(theme, /export type WorkspaceTheme = "loopx" | "paper" | "brutal" | "aios"/, "All supported themes share one stable type");
assert.match(theme, /defaultWorkspaceTheme: WorkspaceTheme = "aios"/, "AI-OS glass theme is the default");
assert.match(theme, /stored === "loopx" || stored === "paper" || stored === "brutal" || stored === "aios"/, "Saved legacy themes remain valid");
...
assert.match(shell, /theme = "loopx"/, "Workspace shell defaults to LoopX standard (component fallback)");
...
assert.ok(settings.includes('onThemeChange("aios")'), "Settings expose AI-OS glass theme");
assert.match(styles, /data-pw-theme="aios"/, "AI-OS glass theme owns scoped styles");
assert.match(styles, /personal-workspace-shell\[data-pw-theme="aios"\] \.personal-workspace-sidebar[\s\S]*backdrop-filter: blur\(14px\)/, "AI-OS sidebar is translucent glass");
assert.ok(styles.includes('background: transparent;'), "AI-OS theme removes opaque shell canvas so the galaxy shows through");
...
assert.match(i18n, /"settings.themeAios": "AI-OS cockpit"/, "English AI-OS theme label is present");
assert.match(i18n, /"settings.themeAios": "AI-OS 面板"/, "Chinese AI-OS theme label is present");
